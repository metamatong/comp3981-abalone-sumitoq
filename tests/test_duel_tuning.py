...
class AdaptiveGauntletTests(unittest.TestCase):
    ...
    def test_run_tuning_loop_is_deterministic_with_fixed_seed(self):
        ...
        with tempfile.TemporaryDirectory() as temp_one, tempfile.TemporaryDirectory() as temp_two, mock.patch.object(
            gauntlet,
            "list_agents",
            return_value=agents,
        ), mock.patch.object(
        ...
        self.assertIn("[Iteration 1/2] W=0 D=1 L=1 capture_diff=-2 partial_searches=2 accepted.", announcements_one)
        self.assertIn("[Iteration 2/2] W=1 D=1 L=0 capture_diff=2 partial_searches=2 accepted.", announcements_one)
        ...
        self.assertIn("Best win rate 50.00% (Iterations, 2)", announcements_one)
